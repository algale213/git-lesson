...
def estimate_second_integral(num_points):
    ...
    #Define range of x and y random values based on integration limits of x, and y = f(x)
    #When x = 0, y = 10. When x = 1, y = 0.099. so use a range of x from [0, 1.0] and y from [0, 10.0]. Because the area of the test space range_x * range_y = (1)(10) = 10,
    #we multiply the preliminary result (count / num_points) (which has a maximum of 1.0) by the scale factor of the test space area, 10.

    x_values = np.random.uniform(0, 1, num_points)        #Generate a numpy array of random values for x coordinates
    y_values = np.random.uniform(0, 10.0, num_points)     #Generate a numpy array of random values for y coordinates


    #Compare y_values against y_threshold = f(x_values)
    y_threshold = 10 / (1 + 100 * np.power(x_values, 2)) # The values of y according to the function
    y_comparison = y_values < y_threshold
    ...
    #Test with some assert statements
    assert np.all(x_values <= 1.0) and np.all(x_values >= 0.0)
    assert np.all(y_values <= 10.0) and np.all(y_values >= 0.0)
    assert np.all(count <= num_points) and np.all(count >= 0.0)

    #Calculate estimate
    integral_estimate = 10 * (count / num_points) # Multipy up the final result by 10 to account for the test space area of 10.
